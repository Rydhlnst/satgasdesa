...
const pdfUpload = { contentType: z.literal("application/pdf"), size: z.coerce.number().int().positive().max(10 * 1024 * 1024), originalName: z.string().trim().regex(/\.pdf$/i, "Dokumen harus berformat PDF.").max(255) };
export const budgetItemRabAttachmentUploadSchema = z.object({ budgetItemId: budgetItemIdSchema, ...pdfUpload });
export const addBudgetItemRabAttachmentSchema = z.object({ budgetItemId: budgetItemIdSchema, storageKey: z.string().trim().min(1).max(255), contentType: z.literal("application/pdf"), sizeBytes: z.coerce.number().int().positive().max(10 * 1024 * 1024), caption: z.string().trim().max(255).optional() });
export const budgetPeriodAttachmentUploadSchema = z.object({ periodId: budgetPeriodIdSchema, ...pdfUpload });
export const addBudgetPeriodAttachmentSchema = z.object({ periodId: budgetPeriodIdSchema, storageKey: z.string().trim().min(1).max(255), contentType: z.literal("application/pdf"), sizeBytes: z.coerce.number().int().positive().max(10 * 1024 * 1024), caption: z.string().trim().max(255).optional() });
export const budgetPeriodAttachmentDownloadSchema = z.object({ periodId: budgetPeriodIdSchema, attachmentId: uuid });
export const createBudgetChangeRequestSchema = z.object({ budgetItemId: budgetItemIdSchema, proposedAmount: money, reason: z.string().trim().min(1).max(5000) });
export const budgetChangeRequestTransitionSchema = z.object({ id: uuid, status: z.enum(["SUBMITTED", "VERIFIED", "APPROVED", "REJECTED", "CANCELLED"]), notes: z.string().trim().max(5000).optional() });
export const budgetChangeRequestAttachmentUploadSchema = z.object({ changeRequestId: uuid, ...pdfUpload });
export const addBudgetChangeRequestAttachmentSchema = z.object({ changeRequestId: uuid, storageKey: z.string().trim().min(1).max(255), contentType: z.literal("application/pdf"), sizeBytes: z.coerce.number().int().positive().max(10 * 1024 * 1024), caption: z.string().trim().max(255).optional() });
export const budgetChangeRequestAttachmentDownloadSchema = z.object({ changeRequestId: uuid, attachmentId: uuid });
