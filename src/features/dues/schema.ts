...
export const createDueSchema = z.object({
  ...
  payerName: z.string().trim().min(1, "Nama pembayar wajib diisi.").max(160),
  amountDue: z.coerce.number().int().positive().max(Number.MAX_SAFE_INTEGER),
  dueDate: calendarDate(),
}).superRefine((value, context) => {
  if (value.dueType === "MONTHLY") {
    if (value.sourceMovementId) context.addIssue({ code: "custom", path: ["sourceMovementId"], message: "Iuran bulanan tidak dapat dikaitkan dengan pergerakan masuk jalan." });
    if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(value.referenceKey)) context.addIssue({ code: "custom", path: ["referenceKey"], message: "Iuran bulanan memerlukan referensi berformat YYYY-MM." });
  }
  if (value.dueType === "ROAD_ENTRY") {
    if (!value.sourceMovementId) context.addIssue({ code: "custom", path: ["sourceMovementId"], message: "Iuran masuk jalan memerlukan sumber pergerakan." });
    if (!value.referenceKey.startsWith("ENTRY-")) context.addIssue({ code: "custom", path: ["referenceKey"], message: "Iuran masuk jalan memerlukan referensi ENTRY." });
  }
});
...
export const reverseDuePaymentSchema = z.object({
  duePaymentId: uuid,
  reason: z.string().trim().min(1, "Alasan pembatalan wajib diisi.").max(5000),
  idempotencyKey: uuid,
});
