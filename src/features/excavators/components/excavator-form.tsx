...
const excavatorFormSchema = z.object({
  ...
  if (value.currentBlockId && !value.entryDate) {
    context.addIssue({ code: "custom", path: ["entryDate"], message: "Tanggal masuk wajib diisi jika blok awal dipilih." });
  }
  if (!value.currentBlockId && value.entryDate) {
    context.addIssue({ code: "custom", path: ["currentBlockId"], message: "Pilih blok awal atau kosongkan tanggal masuk." });
  }
});
...
export function ExcavatorForm({ blocks }: ExcavatorFormProps) {
  const form = useForm({
    ...
    onSubmit: async ({ value }) => { const formData = new FormData(); Object.entries(value).forEach(([key, item]) => formData.set(key, item)); try { await registerExcavatorAction(formData); } catch (error) { showActionError(error); throw error; } },
  });
  ...
  return <Card className="shadow-sm"><CardHeader><CardTitle className="font-heading text-xl">Daftarkan excavator</CardTitle><p className="text-sm text-muted-foreground">Buat data unit terlebih dahulu. Riwayat perpindahan dicatat terpisah.</p></CardHeader><CardContent><form className="space-y-6" noValidate onSubmit={(event) => { event.preventDefault(); event.stopPropagation(); void form.handleSubmit(); }}>
    ...
    <div className="grid gap-5 sm:grid-cols-2">
      ...
      <form.Field name="currentBlockId">{(field) => <div className="space-y-2"><Label htmlFor={field.name}>Blok awal</Label><NativeSelect aria-invalid={field.state.meta.errors.length > 0} id={field.name} name={field.name} value={field.state.value} onBlur={field.handleBlur} onChange={(event) => field.handleChange(event.target.value)}><NativeSelectOption value="">Belum ditugaskan</NativeSelectOption>{blocks.map((block) => <NativeSelectOption key={block.id} value={block.id}>{block.code} · {block.name}</NativeSelectOption>)}</NativeSelect><p className="text-xs text-muted-foreground">Penugasan blok memerlukan tanggal masuk. Kosongkan tanggal jika belum ditugaskan.</p><FieldError errors={field.state.meta.errors} /></div>}</form.Field>
