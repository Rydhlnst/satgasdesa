...
export default function TasksScreen() {
  ...
  return <><Header role={role} title={role === "PETUGAS_LAPANGAN" ? "Tugas Saya" : "Tugas Lapangan"} subtitle="Pemantauan pekerjaan per blok" /><Screen><InteractiveTabs active={statusIndex} items={statuses} onChange={setStatusIndex} /><SearchField value={query} onChangeText={setQuery} onClear={() => setQuery("")} placeholder="Cari tugas..." />{result.isLoading ? <LoadingState /> : result.isError ? <ErrorState message="Tugas lapangan tidak dapat dimuat." onRetry={() => result.refetch()} /> : <View style={styles.list}>{tasks.length ? tasks.map((item) => <RowCard key={text(item, "id")} onPress={() => router.push(`/task/${text(item, "id")}`)} icon={<ClipboardCheck color={colors.primary} size={22} />} title={text(item, "title")} subtitle={`Blok: ${text(item, "blockId")} · Prioritas ${displayStatus(text(item, "priority"))}`} meta={text(item, "dueDate", "Tanpa tenggat")} status={text(item, "status")} tone={text(item, "status") === "DONE" ? "green" : text(item, "status") === "CANCELLED" ? "red" : "orange"} />) : <EmptyState message="Tidak ada tugas pada filter ini." />}</View>}{canCreate ? <ActionButton onPress={() => router.push("/task/new")}>Buat Tugas Lapangan</ActionButton> : null}</Screen><BottomNav current="monitoring" role={role} /></>;
}
