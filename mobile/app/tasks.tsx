...
export default function TasksScreen() {
  ...
  return <><Header role={role} title={role === "PETUGAS_LAPANGAN" ? "Tugas Saya" : "Tugas Lapangan"} subtitle="Pemantauan pekerjaan per blok" /><Screen reserveBottomNavSpace scroll={false}><InteractiveTabs active={statusIndex} items={statuses} onChange={setStatusIndex} /><SearchField value={query} onChangeText={setQuery} onClear={() => setQuery("")} placeholder="Cari tugas..." />{result.isLoading ? <LoadingState /> : result.isError ? <ErrorState message="Tugas lapangan tidak dapat dimuat." error={result.error} onRetry={() => result.refetch()} /> : <FlatList data={tasks} keyExtractor={(item) => text(item, "id")} showsVerticalScrollIndicator={false} contentContainerStyle={styles.list} renderItem={({ item }) => <RowCard onPress={() => router.push(`/task/${text(item, "id")}`)} icon={<ClipboardCheck color={colors.primary} size={20} />} title={text(item, "title")} subtitle={`Blok: ${text(item, "blockId")} · Prioritas ${displayStatus(text(item, "priority"))}`} meta={text(item, "dueDate", "Tanpa tenggat")} status={text(item, "status")} tone={text(item, "status") === "DONE" ? "green" : text(item, "status") === "CANCELLED" ? "red" : "orange"} />} ListEmptyComponent={<EmptyState message="Tidak ada tugas pada filter ini." />} />}{canCreate ? <ActionButton onPress={() => router.push("/task/new")}>Buat Tugas Lapangan</ActionButton> : null}</Screen><BottomNav current="monitoring" role={role} /></>;
}
