...
import { ActionButton, InteractiveTabs } from "../src/components/PimpinanPrimitives";
...
export default function Monitoring() {
  ...
  const canCreate = role === "PIMPINAN";
  const canManageAssignments = role === "PIMPINAN";
  const content = query.isLoading ? <LoadingState /> : query.isError ? <ErrorState message="Daftar blok tidak dapat dimuat." onRetry={() => query.refetch()} /> : <FlatList data={visibleBlocks} keyExtractor={(item) => item.id} ListEmptyComponent={<EmptyState message={statusFilter === "ALL" ? "Belum ada data blok." : "Tidak ada blok pada filter ini."} />} onRefresh={() => query.refetch()} refreshing={query.isRefetching} renderItem={({ item }) => <Pressable onPress={() => router.push(`/blocks/${item.id}`)} style={styles.block}><View style={styles.blockTop}><Text style={styles.code}>{item.code}</Text><Text style={[styles.status, item.status === "ACTIVE" ? styles.active : item.status === "STOPPED" ? styles.inactive : styles.neutral]}>{item.status === "ACTIVE" ? "Aktif" : item.status === "STOPPED" ? "Berhenti" : "Belum Operasi"}</Text></View><Text style={styles.name}>{item.name}</Text><Text style={styles.detail}>Status: {item.status === "ACTIVE" ? "Aktif" : item.status === "STOPPED" ? "Berhenti" : "Belum Operasi"}</Text><Text style={styles.detail}>Pekerja: {item.workerCount}{item.managerName ? ` · ${item.managerName}` : ""}</Text></Pressable>} contentContainerStyle={styles.list} ListFooterComponent={<View style={styles.actions}>{canCreate ? <ActionButton onPress={() => router.push("/block/new")}>Tambah Blok</ActionButton> : null}{canManageAssignments ? <ActionButton onPress={() => router.push("/assignments")}>Kelola Penugasan Blok</ActionButton> : null}<ActionButton onPress={() => router.push("/tasks")}>{role === "PETUGAS_LAPANGAN" ? "Lihat Tugas Saya" : "Kelola Tugas Lapangan"}</ActionButton><ActionButton onPress={() => router.push("/workers")}>Data Pekerja</ActionButton></View>} />;
  return <><Header role={role} title="Monitoring Blok" subtitle="Daftar blok operasional" /><Screen scroll={false}><InteractiveTabs active={tab} items={["Daftar Blok", "Peta", "Excavator", "Pemeriksaan"]} onChange={(index) => { if (index === 1) router.push("/map"); else if (index === 2) router.push("/excavators"); else if (index === 3) router.push("/inspections"); else setTab(0); }} /><TextInput onChangeText={setSearch} placeholder="Cari blok..." placeholderTextColor={colors.textMuted} style={styles.search} value={search} /><InteractiveTabs active={statusFilter === "ALL" ? 0 : statusFilter === "ACTIVE" ? 1 : statusFilter === "STOPPED" ? 2 : 3} items={[`Semua (${counts.all})`, `Aktif (${counts.active})`, `Berhenti (${counts.stopped})`, `Belum Operasi (${blocks.filter((item) => item.status === "NOT_OPERATING").length})`]} onChange={(index) => setStatusFilter(index === 0 ? "ALL" : index === 1 ? "ACTIVE" : index === 2 ? "STOPPED" : "NOT_OPERATING")} />{content}</Screen><BottomNav current="monitoring" role={role} /></>;
}
