...
const schema = z.object({ payerName: z.string().trim().min(1).max(160), paymentDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), amount: z.coerce.number().int().positive(), method: z.enum(["CASH", "BANK_TRANSFER", "QRIS", "OTHER"]), notes: z.string().max(5000).optional() });
