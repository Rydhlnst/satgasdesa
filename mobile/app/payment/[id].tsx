...
export default function PaymentForm() {
  ...
  async function uploadEvidence() { if (!evidence) return undefined; const size = evidence.fileSize ?? 0; if (!size) throw new Error("Ukuran bukti tidak tersedia."); if (size > 10 * 1024 * 1024) throw new Error("Bukti pembayaran maksimal 10 MB."); const contentType = evidence.mimeType ?? "image/jpeg"; const upload = await createDuePaymentUploadUrl({ dueId, paymentId, contentType, size, originalName: evidence.fileName ?? `pembayaran-${Date.now()}.jpg` }); const response = await fetch(upload.uploadUrl, { method: "PUT", headers: { "Content-Type": contentType }, body: await (await fetch(evidence.uri)).blob() }); if (!response.ok) throw new Error("Unggah bukti pembayaran gagal."); return upload.key; }
