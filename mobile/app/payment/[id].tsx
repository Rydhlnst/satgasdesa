...
export default function PaymentForm() {
  const { id } = useLocalSearchParams<{ id: string }>(); const { role } = useAuth(); const router = useRouter(); const [saving, setSaving] = useState(false); const [paymentId] = useState(createClientId); const [evidence, setEvidence] = useState<ImagePicker.ImagePickerAsset | null>(null);
...
const styles = StyleSheet.create({ balance: { color: colors.finance, fontSize: 15, fontWeight: "900", marginBottom: spacing.sm }, helper: { color: colors.textMuted, fontSize: 11, lineHeight: 16 }, evidence: { alignItems: "center", borderColor: colors.primary, borderRadius: 10, borderWidth: 1, padding: 13 }, evidenceText: { color: colors.primary, fontSize: 12, fontWeight: "900" }, preview: { borderRadius: 10, height: 120, width: 120 } });
