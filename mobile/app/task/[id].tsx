...
export default function TaskDetailScreen() {
  ...
  async function transition(next: string) { if (saving) return; const parsed = taskStatusFormSchema.safeParse({ id, status: next }); if (!parsed.success) return Alert.alert("Status tugas tidak valid", parsed.error.issues[0]?.message ?? "Muat ulang tugas lalu coba lagi."); setSaving(true); try { await updateFieldTask(parsed.data); await client.invalidateQueries({ queryKey: ["field-task", id] }); await client.invalidateQueries({ queryKey: ["field-tasks"] }); } catch (error) { showActionError(error, "Coba lagi."); } finally { setSaving(false); } }
