...
export default function ProfileSecurity() {
  ...
  return <><Header role={role} title="Password and sessions" subtitle="Secure this account" /><Screen><View style={styles.card}><Text style={styles.copy}>Use a unique password. You can revoke other active sessions after updating it.</Text><InputField name="currentPassword" label="Current password" register={form.register} errors={form.formState.errors} secureTextEntry /><InputField name="newPassword" label="New password" register={form.register} errors={form.formState.errors} secureTextEntry /><InputField name="confirmPassword" label="Confirm new password" register={form.register} errors={form.formState.errors} secureTextEntry /><SelectField label="Other sessions" value={form.watch("revokeOtherSessions")} options={[{ label: "Revoke", value: "yes" }, { label: "Keep active", value: "no" }]} onChange={(value) => form.setValue("revokeOtherSessions", value as "yes" | "no", { shouldValidate: true })} /><SubmitButton label="Change password" loading={form.formState.isSubmitting} onPress={() => void form.handleSubmit(submit)()} /></View><Pressable onPress={() => void signOut()} style={styles.signOut}><Text style={styles.signOutText}>Sign out from this device</Text></Pressable></Screen></>;
}
