...
export default function NewMonthlyDue() {
  const { role } = useAuth(); const router = useRouter(); const queryClient = useQueryClient(); const [saving, setSaving] = useState(false); const [saveError, setSaveError] = useState<unknown>(null); const periodKey = new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Jakarta", year: "numeric", month: "2-digit" }).format(new Date());
  ...
  async function submit(values: Values) { setSaveError(null); setSaving(true); try { await createDue({ ...values, dueType: "MONTHLY", referenceKey: periodKey }); await queryClient.invalidateQueries({ queryKey: ["dues"] }); await queryClient.invalidateQueries({ queryKey: ["finance-summary"] }); Alert.alert("Tersimpan", "Iuran bulanan berhasil dibuat.", [{ text: "OK", onPress: () => router.back() }]); } catch (error) { setSaveError(error); } finally { setSaving(false); } }
