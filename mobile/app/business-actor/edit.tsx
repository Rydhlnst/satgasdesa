...
export default function EditBusinessActor() {
  ...
  if (!role) return null;
  async function submit(values: Values) { if (!params.id) return; setSaving(true); try { await updateBusinessActor({ ...values, id: params.id }); Alert.alert("Berhasil", "Pelaku usaha berhasil diperbarui.", [{ text: "OK", onPress: () => router.back() }]); } catch (error) { showActionError(error, "Periksa data."); } finally { setSaving(false); } }
