...
export default function EditBusinessActor() {
  const { role } = useAuth(); const router = useRouter(); const queryClient = useQueryClient(); const params = useLocalSearchParams<{ id: string; actorType?: string; name?: string; representativeName?: string; contact?: string; address?: string; notes?: string }>(); const [saving, setSaving] = useState(false);
  ...
  if (!role) return null;
  async function submit(values: Values) { if (!params.id) return; setSaving(true); try { await updateBusinessActor({ ...values, id: params.id }); await queryClient.invalidateQueries({ queryKey: ["business-actors"] }); Alert.alert("Berhasil", "Pelaku usaha berhasil diperbarui.", [{ text: "OK", onPress: () => router.back() }]); } catch (error) { showActionError(error, "Periksa data."); } finally { setSaving(false); } }
