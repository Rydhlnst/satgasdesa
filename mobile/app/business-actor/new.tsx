...
export default function NewBusinessActor() {
  const { role } = useAuth(); const router = useRouter(); const queryClient = useQueryClient(); const [saving, setSaving] = useState(false);
  ...
  if (!role) return null;
  async function submit(values: Values) { setSaving(true); try { await createBusinessActor(values); await queryClient.invalidateQueries({ queryKey: ["business-actors"] }); Alert.alert("Berhasil", "Pelaku usaha berhasil ditambahkan.", [{ text: "OK", onPress: () => router.back() }]); } catch (error) { showActionError(error, "Periksa data."); } finally { setSaving(false); } }
