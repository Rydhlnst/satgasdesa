...
export default function BusinessActors() {
  ...
  if (!role) return null;
  return <><Header role={role} title="Pelaku Usaha" subtitle="Master data pelaku usaha" /><Screen><SearchField value={query} onChangeText={setQuery} onClear={() => setQuery("")} placeholder="Cari nama atau kontak" />{canManage ? <ActionButton onPress={() => router.push("/business-actor/new")}>Tambah pelaku usaha</ActionButton> : null}{actors.isLoading ? <LoadingState /> : actors.isError ? <ErrorState message="Data pelaku usaha tidak dapat dimuat." onRetry={() => actors.refetch()} /> : actors.data?.actors.length ? actors.data.actors.map((actor) => <RowCard key={text(actor, "id")} onPress={canManage ? () => router.push({ pathname: "/business-actor/edit", params: { id: text(actor, "id"), actorType: text(actor, "actorType"), name: text(actor, "name"), representativeName: text(actor, "representativeName"), contact: text(actor, "contact"), address: text(actor, "address"), notes: text(actor, "notes") } }) : undefined} icon={<Building2 color={colors.primary} size={22} />} title={text(actor, "name")} subtitle={displayStatus(text(actor, "actorType"))} meta={text(actor, "contact", "Tidak ada kontak")} />) : <EmptyState message="Belum ada pelaku usaha." />}</Screen><BottomNav current="profile" role={role} /></>;
}
