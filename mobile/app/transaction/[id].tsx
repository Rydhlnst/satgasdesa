...
export default function TransactionDetail() {
  ...
  async function reverse() { const parsed = reversalFormSchema.safeParse({ reason }); if (!parsed.success) return Alert.alert("Alasan diperlukan", parsed.error.issues[0]?.message ?? "Masukkan alasan pembalikan transaksi."); try { await reverseFinancialTransaction({ id, ...parsed.data }); setReason(""); await refresh(); } catch (error) { Alert.alert("Tidak dapat membalikkan", error instanceof Error ? error.message : "Periksa koneksi lalu coba lagi."); } }
