...
const labels = ["Semua", "Draft", "Diajukan", "Diverifikasi", "Sah", "Revisi", "Ditolak", "Dibatalkan"];
...
export default function Proposals() {
  ...
  const [periodKey, setPeriodKey] = useState(new Date().toISOString().slice(0, 7)); const [statusIndex, setStatusIndex] = useState(0); const [query, setQuery] = useState(""); const [filtersOpen, setFiltersOpen] = useState(false); const [categoryId, setCategoryId] = useState(""); const [blockId, setBlockId] = useState(""); const [mine, setMine] = useState(false);
  ...
  const result = useQuery({ queryKey: ["fund-requests", periodKey, statuses[statusIndex], query, categoryId, blockId, mine], queryFn: () => getFundRequests({ periodKey, status: statuses[statusIndex], query: query.trim(), categoryId, blockId, mine: mine ? "true" : "" }), enabled: Boolean(role) });
  if (!role) return null;
