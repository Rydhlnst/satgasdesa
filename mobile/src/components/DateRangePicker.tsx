...
export function DateRangePicker() {
  ...
  return <>
    <Pressable accessibilityLabel="Change date range" accessibilityRole="button" onPress={openPicker} style={styles.bar}>
      <View style={styles.labelRow}><CalendarDays color={colors.primary} size={16} /><Text style={styles.label}>DATE RANGE</Text></View>
      <Text style={styles.value}>{range.dateFrom} — {range.dateTo}</Text>
    </Pressable>
...
const styles = StyleSheet.create({ bar: { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: 10, borderWidth: 1, gap: 3, paddingHorizontal: 11, paddingVertical: 8 }, labelRow: { alignItems: "center", flexDirection: "row", gap: 5 }, label: { color: colors.primary, fontSize: 9, fontWeight: "900", letterSpacing: 0.4 }, value: { color: colors.text, fontSize: 11, fontWeight: "800" }, backdrop: { backgroundColor: "#00000066", flex: 1, justifyContent: "flex-end" }, sheet: { backgroundColor: colors.surface, borderTopLeftRadius: 20, borderTopRightRadius: 20, gap: spacing.sm, padding: spacing.lg }, title: { color: colors.text, fontSize: 18, fontWeight: "900" }, hint: { color: colors.textMuted, fontSize: 11, lineHeight: 16 }, fieldLabel: { color: colors.text, fontSize: 11, fontWeight: "800", marginTop: spacing.xs }, input: { borderColor: colors.border, borderRadius: 10, borderWidth: 1, color: colors.text, height: 44, paddingHorizontal: 12 }, error: { color: colors.danger, fontSize: 10 }, quick: { alignItems: "center", borderColor: colors.primary, borderRadius: 9, borderWidth: 1, padding: 11 }, quickText: { color: colors.primary, fontSize: 11, fontWeight: "800" }, actions: { flexDirection: "row", gap: spacing.sm, justifyContent: "flex-end", marginTop: spacing.sm }, cancel: { borderColor: colors.border, borderRadius: 9, borderWidth: 1, padding: spacing.md }, cancelText: { color: colors.text, fontWeight: "800" }, apply: { backgroundColor: colors.primary, borderRadius: 9, padding: spacing.md }, applyText: { color: "#FFFFFF", fontWeight: "800" } });
