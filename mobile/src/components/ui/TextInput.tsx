...
const CurrencyTextInput = forwardRef<React.ElementRef<typeof InputField>, TextInputProps>(function CurrencyTextInput({ style, value, onChangeText, onFocus, onBlur, ...props }, ref) {
  ...
  return <Input style={style} className="min-h-12 rounded-xl border-[#DFE4EC] bg-white px-3"><InputField ref={ref} {...props} keyboardType={props.keyboardType ?? "numeric"} value={focused ? String(value ?? "") : formatMoneyInput(value as string | number | null | undefined)} onChangeText={(next) => onChangeText?.(parseMoneyInput(next))} onFocus={(event: NativeSyntheticEvent<TextInputFocusEventData>) => { setFocused(true); onFocus?.(event); }} onBlur={(event: NativeSyntheticEvent<TextInputFocusEventData>) => { setFocused(false); onBlur?.(event); }} className={props.multiline ? "min-h-28 py-3" : "py-3"} /></Input>;
});
