...
import { InlineErrorPanel } from "./OperationalPrimitives";
...
function FieldShell({ label, required, error, helper, children }: { label: string; required?: boolean; error?: unknown; helper?: string; children: React.ReactNode }) {
  ...
  return <FormControl isInvalid={Boolean(message)} isRequired={required} className="gap-0.5">
...
function ControlledRHFInputField<T extends FieldValues>({ name, label, register, errors, multiline = false, keyboardType = "default", placeholder, secureTextEntry = false, required = false, helper, currency = false, control }: RHFInputProps<T> & { control: Control<T> }) {
  ...
  const message = errorMessage(errors[name as keyof T]);
  const displayValue = currency ? formatMoneyInput(watchedValue as unknown as string | number | null | undefined) : String(watchedValue ?? "");
  const resolvedPlaceholder = inputPlaceholder({ label, placeholder, multiline, keyboardType, secureTextEntry, currency });
  return <FieldShell label={label} required={required} error={message} helper={helper}><Input isInvalid={Boolean(message)} className="min-h-12 rounded-xl border-[#DFE4EC] bg-white px-3"><InputField accessibilityLabel={label} autoCapitalize={keyboardType === "email-address" ? "none" : undefined} keyboardType={keyboardType} multiline={multiline} numberOfLines={multiline ? 3 : 1} onBlur={registered.onBlur} onChangeText={(value) => void registered.onChange({ target: { name, value: currency ? parseMoneyInput(value) : value }, type: "change" })} placeholder={resolvedPlaceholder} placeholderTextColor="#8A96A8" ref={(node) => registered.ref(node)} secureTextEntry={secureTextEntry && !visible} value={displayValue} className={multiline ? "min-h-24 py-3" : "py-3"} />{secureTextEntry ? <InputSlot accessibilityLabel={visible ? "Sembunyikan " + label : "Tampilkan " + label} accessibilityRole="button" onPress={() => setVisible((current) => !current)} className="min-h-11 min-w-10">{visible ? <EyeOff color="#6E7785" size={19} /> : <Eye color="#6E7785" size={19} />}</InputSlot> : null}</Input></FieldShell>;
}
...
function RegularTextInputField({ label, value, onChange, error, multiline = false, keyboardType = "default", placeholder, required = false, helper, secureTextEntry = false, currency = false }: { label: string; value: string | number | null | undefined; onChange: (value: string) => void; error?: unknown; multiline?: boolean; keyboardType?: KeyboardType; placeholder?: string; required?: boolean; helper?: string; secureTextEntry?: boolean; currency?: boolean }) {
  const message = errorMessage(error);
  const [visible, setVisible] = useState(false);
  const displayValue = currency ? formatMoneyInput(value) : String(value ?? "");
  const resolvedPlaceholder = inputPlaceholder({ label, placeholder, multiline, keyboardType, secureTextEntry, currency });
  return <FieldShell label={label} required={required} error={message} helper={helper}><Input isInvalid={Boolean(message)} className="min-h-12 rounded-xl border-[#DFE4EC] bg-white px-3"><InputField accessibilityLabel={label} autoCapitalize={keyboardType === "email-address" ? "none" : undefined} keyboardType={keyboardType} multiline={multiline} numberOfLines={multiline ? 3 : 1} onChangeText={(next) => onChange(currency ? parseMoneyInput(next) : next)} placeholder={resolvedPlaceholder} placeholderTextColor="#8A96A8" value={displayValue} secureTextEntry={secureTextEntry && !visible} className={multiline ? "min-h-24 py-3" : "py-3"} />{secureTextEntry ? <InputSlot accessibilityLabel={visible ? "Sembunyikan " + label : "Tampilkan " + label} accessibilityRole="button" onPress={() => setVisible((current) => !current)} className="min-h-11 min-w-10">{visible ? <EyeOff color="#6E7785" size={19} /> : <Eye color="#6E7785" size={19} />}</InputSlot> : null}</Input></FieldShell>;
}
...
export function SubmitButton({ label, loading, loadingLabel = "Menyimpan…", onPress }: { label: string; loading: boolean; loadingLabel?: string; onPress: () => void | Promise<void> }) {
  ...
  async function handlePress() { if (busy || inFlight.current) return; inFlight.current = true; setActionError(undefined); setPending(true); try { await onPress(); } catch (error) { setActionError(error); showActionError(error); } finally { inFlight.current = false; setPending(false); } }
  return <>{actionError ? <InlineErrorPanel error={actionError} fallback="Perubahan belum tersimpan. Periksa data dan koneksi lalu coba lagi." onRetry={() => void handlePress()} /> : null}<Button accessibilityRole="button" accessibilityState={{ busy, disabled: busy }} disabled={busy} onPress={() => void handlePress()} className="min-h-12 rounded-xl bg-[#1454C4]">{busy ? <ButtonSpinner color="#FFFFFF" /> : null}<ButtonText>{busy ? loadingLabel : label}</ButtonText></Button></>;
}
