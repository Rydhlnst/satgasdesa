...
export function CalendarSheet({ open, onClose, value, onChange, title, mode = "date" }: { open: boolean; onClose: () => void; value: string; onChange: (value: string) => void; title: string; mode?: CalendarMode }) {
  ...
  return <Modal isOpen={open} onClose={onClose} size="full">
    ...
    <ModalContent className="mt-auto w-full rounded-t-3xl rounded-b-none p-5 pb-safe">
      <ModalHeader><View style={styles.titleRow}><View style={styles.titleIcon}><CalendarDays color={colors.primary} size={18} /></View><Text style={styles.title}>{title}</Text></View></ModalHeader>
      <ModalBody>
        <View style={styles.calendarHeader}><Pressable accessibilityLabel="Previous month" accessibilityRole="button" hitSlop={8} onPress={() => mode === "date" ? moveMonth(-1) : moveYear(-1)} style={styles.navButton}><ChevronLeft color={colors.textStrong} size={18} /></Pressable><Text style={styles.monthTitle}>{mode === "date" ? `${monthNames[visible.getMonth()]} ${visible.getFullYear()}` : String(visible.getFullYear())}</Text><Pressable accessibilityLabel="Next month" accessibilityRole="button" hitSlop={8} onPress={() => mode === "date" ? moveMonth(1) : moveYear(1)} style={styles.navButton}><ChevronRight color={colors.textStrong} size={18} /></Pressable></View>
...
const styles = StyleSheet.create({ titleRow: { alignItems: "center", flexDirection: "row", gap: spacing.sm }, titleIcon: { alignItems: "center", backgroundColor: colors.primarySoft, borderRadius: radii.md, height: 34, justifyContent: "center", width: 34 }, title: { color: colors.textStrong, fontSize: 18, fontWeight: "900" }, calendarHeader: { alignItems: "center", flexDirection: "row", justifyContent: "space-between", marginBottom: spacing.md }, navButton: { alignItems: "center", backgroundColor: colors.surfaceMuted, borderRadius: radii.pill, height: 36, justifyContent: "center", width: 36 }, monthTitle: { color: colors.textStrong, fontSize: 15, fontWeight: "900" }, weekdays: { flexDirection: "row", marginBottom: 4 }, weekday: { color: colors.textMuted, flex: 1, fontSize: 10, fontWeight: "800", textAlign: "center" }, grid: { flexDirection: "row", flexWrap: "wrap" }, dayCell: { alignItems: "center", height: 44, justifyContent: "center", width: `${100 / 7}%` }, dayCircle: { alignItems: "center", borderRadius: radii.pill, height: 34, justifyContent: "center", width: 34 }, selectedCircle: { backgroundColor: colors.primary }, dayText: { color: colors.textStrong, fontSize: 12, fontWeight: "700" }, selectedText: { color: "#FFFFFF", fontWeight: "900" }, monthGrid: { flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }, monthCell: { alignItems: "center", borderColor: colors.border, borderRadius: radii.md, borderWidth: 1, height: 44, justifyContent: "center", width: "23%" }, selectedMonth: { backgroundColor: colors.primary, borderColor: colors.primary }, monthText: { color: colors.textStrong, fontSize: 11, fontWeight: "800" }, error: { color: colors.danger, fontSize: 11, marginTop: spacing.sm } });
