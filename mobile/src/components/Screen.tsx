...
export function BottomNav({ role, current }: { role: Role; current: string }) {
  ...
  if (role === "PENGUSAHA") return <View style={navStyle}><NavItem icon={Home} label="Beranda" active={current === "dashboard"} href="/dashboard" /><NavItem icon={MapPinned} label="Blok" active={current === "monitoring"} href="/monitoring" /><NavItem icon={WalletCards} label="Iuran" active={current === "business-portal"} href="/business-portal" /><NavItem icon={Info} label="Informasi" active={current === "information"} href="/information" /><NavItem icon={CircleUserRound} label="Profil" active={current === "profile"} href="/profile" /></View>;
...
function NavItem({ label, active, href, icon: Icon }: { label: string; active: boolean; href: "/dashboard" | "/blocks" | "/monitoring" | "/business-portal" | "/finance" | "/information" | "/reports" | "/budgets" | "/proposals" | "/realizations" | "/inspections" | "/excavators" | "/profile" | "/notifications" | "/more"; icon: typeof Home }) {
  const router = useRouter();
