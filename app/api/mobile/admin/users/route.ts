...
const createUserRequestSchema = z.object({ name: z.string().trim().min(2).max(255), email: z.string().trim().email().max(255), roleId: z.enum(["PIMPINAN", "BENDAHARA", "PETUGAS_LAPANGAN", "PENGUSAHA"]), password: z.string().min(8).max(128) });
