...
export async function GET(request: Request) {
  return withMobileSession(request, async () => {
    try {
      ...
      const result = await getDuesPage({ status: params.get("status") || undefined, dueType: params.get("dueType") || undefined, blockId: params.get("blockId") || undefined, periodKey: params.get("periodKey") || undefined, dateFrom: params.get("dateFrom") || undefined, dateTo: params.get("dateTo") || undefined, query: params.get("query") || undefined, overdueOnly: params.get("overdueOnly") === "true", page: params.get("page") || undefined, pageSize: params.get("pageSize") || undefined });
      const dues = result.rows.map((row) => "due" in row && row.due ? { ...row.due, excavator: row.excavator, block: row.block, businessActor: row.business_actor } : row);
