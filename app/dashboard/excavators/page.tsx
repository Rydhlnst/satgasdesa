...
import { MobileExcavatorScreen } from "../_components/mobile-operational-screens";
...
export default async function ExcavatorsPage({ searchParams }: ExcavatorsPageProps) {
  ...
  const mobileRows = result.rows.map((row) => ({ id: row.excavator.id, unitCode: row.excavator.unitCode, brand: row.excavator.brand, model: row.excavator.model, operatorName: row.excavator.operatorName, status: row.excavator.status, blockCode: row.block?.code ?? null }));
  const hasFilters = Boolean(params.query?.trim() || params.status || params.blockId);
  ...
  return (
    <PageContainer>
      <div className="space-y-8">
        ...
        <form className="hidden gap-3 border-y border-border py-4 sm:grid-cols-[minmax(0,1fr)_180px_220px_auto] md:grid" method="get">
        ...
        {result.pagination.total === 0 ? <section className="hidden rounded-xl border border-border bg-card shadow-sm md:block">{hasFilters ? <NoResultsState description="Coba ubah kata kunci atau hapus filter untuk melihat unit lain." icon={HardHat} title="Excavator tidak ditemukan" /> : <EmptyState description="Daftarkan excavator pertama untuk mulai mencatat unit dan pergerakannya." icon={HardHat} title="Belum ada excavator" />}</section> : (
          <>
        ...
        <MobileExcavatorScreen canManage={canManage} rows={mobileRows} />
      </div>
    </PageContainer>
  );
}
