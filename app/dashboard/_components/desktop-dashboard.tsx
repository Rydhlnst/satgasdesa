...
function AttentionList({ items }: { items: AttentionItem[] }) {
  return <div className="divide-y divide-[#edf0f4]">{items.length ? items.slice(0, 4).map((item) => <article className="flex gap-3 px-5 py-4" key={`${item.entityType}-${item.entityId}-${item.type}`}><span className={`mt-0.5 grid size-7 shrink-0 place-items-center rounded-full ${item.severity === "HIGH" ? "bg-[#ffe9e7] text-[#c5312c]" : "bg-[#fff3e5] text-[#d87914]"}`}><Info className="size-3.5" /></span><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-bold text-[#263959]">{item.title}</p><StatusBadge status={item.severity} /></div><p className="mt-1 text-xs leading-relaxed text-[#718096]">{item.detail}</p></div></article>) : <p className="px-5 py-6 text-sm text-[#718096]">Tidak ada data yang perlu ditindaklanjuti.</p>}</div>;
}
