...
export default async function TransactionDetailPage({ params }: Props) {
  ...
  return <PageContainer><div className="space-y-8"><PageHeader breadcrumbs={[{ label: "Dashboard", href: "/dashboard" }, { label: "Finance", href: "/dashboard/finance" }, { label: transaction.transactionCode }]} description={transaction.description} eyebrow="Transaction detail" title={transaction.transactionCode} actions={<StatusBadge status={transaction.status} />} /><section className="grid gap-4 sm:grid-cols-3"><Info label="Type" value={transaction.transactionType.replace("CASH_", "Cash ")} /><Info label="Amount" value={<MoneyDisplay value={transaction.amount} />} /><Info label="Recorded" value={transaction.transactionAt.toLocaleString("en-GB")} /></section><div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]"><Card className="shadow-sm"><CardHeader><CardTitle className="font-heading text-xl uppercase tracking-wide">Evidence</CardTitle></CardHeader><CardContent className="space-y-5">{evidence.length ? <EvidenceList entityId={id} items={evidence} kind="transaction" /> : <p className="text-sm text-muted-foreground">No evidence attached.</p>}{canCreate && transaction.status === "DRAFT" ? <EvidenceUploader entityId={id} kind="transaction" /> : null}</CardContent></Card><div className="space-y-5">{canApprove && transaction.status === "DRAFT" ? <Card><CardHeader><CardTitle>Approve</CardTitle></CardHeader><CardContent><ActionForm action={approveFinancialTransactionAction}><input name="id" type="hidden" value={id} /><Button type="submit">Approve transaction</Button></ActionForm></CardContent></Card> : null}{canApprove && transaction.status === "SAH" ? <Card><CardHeader><CardTitle>Reverse</CardTitle></CardHeader><CardContent><ActionForm action={reverseFinancialTransactionAction} className="space-y-3"><input name="id" type="hidden" value={id} /><textarea className="min-h-24 w-full border border-input bg-transparent p-3 text-sm" name="reason" placeholder="Reversal reason" required /><Button type="submit" variant="destructive">Reverse transaction</Button></ActionForm></CardContent></Card> : null}</div></div></div></PageContainer>;
}
