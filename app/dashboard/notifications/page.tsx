...
function notificationTarget(item: { relatedEntityType: string | null; relatedEntityId: string | null }): string | null {
  if (item.relatedEntityType === "BUDGET_PERIOD") return "/dashboard/budgets";
  if (!item.relatedEntityType || !item.relatedEntityId) return null;
  switch (item.relatedEntityType) {
    case "BLOCK": return `/dashboard/blocks/${item.relatedEntityId}`;
    case "INSPECTION": return `/dashboard/inspections/${item.relatedEntityId}`;
    case "DAILY_INFORMATION": return `/dashboard/information/${item.relatedEntityId}`;
    case "DUE": return `/dashboard/dues/${item.relatedEntityId}`;
    case "REALIZATION": return `/dashboard/realizations/${item.relatedEntityId}`;
    case "FINANCIAL_TRANSACTION": return `/dashboard/finance/transactions/${item.relatedEntityId}`;
    case "FUND_REQUEST": return `/dashboard/fund-requests/${item.relatedEntityId}`;
    case "EXCAVATOR": return `/dashboard/excavators/${item.relatedEntityId}`;
    case "DUE_PAYMENT": return "/dashboard/payments";
