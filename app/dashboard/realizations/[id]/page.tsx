...
export default async function RealizationDetailPage({ params }: Props) {
  ...
  return <PageContainer><div className="space-y-8">
    ...
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <div className="space-y-5">{transitions.length && (canCreate || canVerify || canApprove) ? <Card><CardHeader><CardTitle>Workflow action</CardTitle></CardHeader><CardContent><ActionForm action={transitionRealizationAction} className="space-y-3"><input name="id" type="hidden" value={id} /><select className="h-10 w-full border-b border-input bg-transparent" name="status">{transitions.map((status) => <option key={status} value={status}>{status}</option>)}</select><textarea className="min-h-24 w-full border border-input bg-transparent p-3 text-sm" name="notes" placeholder="Notes or rejection reason" required={transitions.some((status) => status === "REJECTED")} /><Button type="submit">Apply transition</Button></ActionForm></CardContent></Card> : null}{canCreate && result.realization.status === "SAH" ? <Card><CardHeader><CardTitle>Correction request</CardTitle></CardHeader><CardContent><ActionForm action={correctRealizationAction} className="space-y-3"><input name="id" type="hidden" value={id} /><input className="h-10 w-full border-b border-input bg-transparent" name="requestedAmount" placeholder="Corrected amount" type="number" min="1" required /><textarea className="min-h-20 w-full border border-input bg-transparent p-3 text-sm" name="description" placeholder="Corrected description" required /><textarea className="min-h-20 w-full border border-input bg-transparent p-3 text-sm" name="reason" placeholder="Correction reason" required /><Button type="submit" variant="outline">Create correction</Button></ActionForm></CardContent></Card> : null}{canApprove && result.realization.status === "SAH" ? <Card><CardHeader><CardTitle>Reverse realization</CardTitle></CardHeader><CardContent><ActionForm action={reverseRealizationAction} className="space-y-3"><input name="id" type="hidden" value={id} /><textarea className="min-h-20 w-full border border-input bg-transparent p-3 text-sm" name="reason" placeholder="Reversal reason" required /><Button type="submit" variant="destructive">Reverse realization</Button></ActionForm></CardContent></Card> : null}</div>
    </div>
  </div></PageContainer>;
}
