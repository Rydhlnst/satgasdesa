...
export function PageSkeleton({ variant = "list" }: PageSkeletonProps) {
  return <PageContainer><div className="space-y-8"><HeaderSkeleton />{variant === "dashboard" ? <><MetricSkeletons /><div className="grid gap-6 lg:grid-cols-2"><div className="h-72 rounded-xl border border-border bg-card p-6"><Skeleton className="h-5 w-40" /><Skeleton className="mt-6 h-44 w-full" /></div><div className="h-72 rounded-xl border border-border bg-card p-6"><Skeleton className="h-5 w-40" /><div className="mt-6 space-y-4">{Array.from({ length: 4 }, (_, index) => <Skeleton className="h-8 w-full" key={index} />)}</div></div></div></> : null}{variant === "list" ? <ListSkeleton /> : null}{variant === "table" ? <TableSkeleton /> : null}{variant === "detail" ? <DetailSkeleton /> : null}{variant === "form" ? <FormSkeleton /> : null}</div></PageContainer>;
}
